...
# ║  - Others   → agri_guard_brain_v2.keras, 38 classes, sliced by index  ║
...
def load_cassava_model():
    return tf.keras.models.load_model("agri_guard_brain_v2.keras")
    ...
    with col_right:
        ...
        if not uploaded:
            ...
            if not leaf_ok:
                ...
                if result is None:
                    st.error("Brain model not available. Please check agri_guard_brain_v2.keras is in the app folder.")
                else:
                    diagnosis  = result["diagnosis"]
                    confidence = result["confidence"]
